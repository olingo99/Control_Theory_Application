...
def Run_PID_Interactive(gamma,alpha,Kp,Kd,Tlead1,Tlag1,Tlead2,Tlag2,Theta1,Theta2,DV0,MV0,PV0, ManFF, FF):
    ...
    The function "Run_PID_Interactive" displays graphs of a PID controller calculated using the '
...
class ProcessPlus:
    ...
    def __init__(self, parameters):
        ...
        self.parameters['Name'] = parameters['Name'] if 'Name' in parameters else "None"
        
def MultipleBode(ListP,omega):
    """
    Based on "Bode" function in package_DBR
    
    :ListP: List of Process as defined by ProcessPlus class
    :omega: frequency vector (rad/s); generated by a command of the type "omega = np.logspace(-2, 2, 10000)".
    
    Displays Bode diagram of avery process in list on the same plot.
    """
    ...
    for P in ListP:
        ...
        # Gain part
        ax_gain.semilogx(omega,20*np.log10(np.abs(Ps)),label=P.parameters['Name']) 
        gain_min = np.min(20*np.log10(np.abs(Ps)/5))
        gain_max = np.max(20*np.log10(np.abs(Ps)*5))
        ax_gain.set_xlim([np.min(omega), np.max(omega)])
        ax_gain.set_ylim([gain_min, gain_max])
        ...
        # Phase part
        ax_phase.semilogx(omega, (180/np.pi)*np.unwrap(np.angle(Ps)),label=P.parameters['Name'])  
        ax_phase.set_xlim([np.min(omega), np.max(omega)])
        ph_min = np.min((180/np.pi)*np.unwrap(np.angle(Ps))) - 10
        ph_max = np.max((180/np.pi)*np.unwrap(np.angle(Ps))) + 10
        ax_phase.set_ylim([np.max([ph_min, -200]), ph_max])
